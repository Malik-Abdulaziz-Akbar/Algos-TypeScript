export const knapsackRecursive = (weights: number[], items: number[], capacity: number, n: number): number => {
    if (n === 0 || capacity === 0) {
        return 0;
    }
...
export const knapsackRecursiveMemoized = (weights: number[], items: number[], capacity: number, n: number, dp: number[][]): number => {
    if (n === 0 || capacity === 0) {
        return 0;
    }
    if (dp[n][capacity] !== -1) {
...
export const knapsackTopDown = (weights: number[], items: number[], capacity: number, n: number): number => {
